...
function main({ contextID, glslVersion }) {
  const {
    GPUComposer,
    GPUProgram,
    GPULayer,
  ...
  const touch = new GPUProgram(composer, {
    ...
    fragmentShader: `
		...
		void main() {
			...
			vec2 velocity = texture(u_velocity, v_uv).xy + (1.0 - radiusSq) * u_vector * ${TOUCH_FORCE_SCALE.toFixed(
        1
      )};
			float velocityMag = length(velocity);
			out_velocity = velocity / velocityMag * min(velocityMag, ${MAX_VELOCITY.toFixed(
        1
      )});
		}`,
    uniforms: [
